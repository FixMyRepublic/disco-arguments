...
_Wikiargument.prototype.raiseNotice = function (msg, callback) {
    ...
    notice.dialog({
        ...
        create: function (event, ui) {
            ...
            $(".ui-dialog-buttonset button").html("Close");
        },
    });
    return;
};
...
_Wikiargument.prototype.submitArgument = function (formId, buttonId) {
    ...
    if (headline == "") {
        this.raiseError('You have to enter a headline.');
        return false;
    } else if (abstract == "") {
        this.raiseError('Please compose an abstract.');
        return false;
    }
...
_Wikiargument.prototype.submitSearch = function (sort) {
    ...
    var root = 'http://en.wikiarguments.net/';
    if (this.group) {
...
_Wikiargument.prototype.passRequest = function () {
    ...
    if (!username) {
        this.raiseError('You have to enter your username.');
        return false;
    }
...
_Wikiargument.prototype.deleteGroup = function () {
    this.raisePrompt("Do you really want to delete this group?", function () { $("#form_delete_group").submit(); }, function () { return false; });
    return false;
};

_Wikiargument.prototype.changeOwnership = function () {
    this.raisePrompt("Do you really want to change the owner-rights?", function () { $("#form_change_ownership").submit(); }, function () { return false; });
};
...
_Wikiargument.prototype.initLocalVars = function () {
    ...
    this.langVars['NOTICE_UNFOLLOW_SUCCESS'] = 'You don\'t follow this question anymore.';
    this.langVars['ERROR_UNFOLLOW_NOT_LOGGED_IN'] = 'ERROR_UNFOLLOW_NOT_LOGGED_IN';
    this.langVars['NOTICE_FOLLOW_SUCCESS'] = 'You are now following this question. This means, that this question is listed on your user-profile and you get an email if there are new comments.';
    this.langVars['ERROR_FOLLOW_NOT_LOGGED_IN'] = 'ERROR_FOLLOW_NOT_LOGGED_IN';
    this.langVars['NOTICE_VOTE_NOT_LOGGED_IN'] = 'You have to be logged-in to vote.';
};
...
jQuery(document).ready(function () {
    ...
    if ($('#form_new_group').length > 0) {
        $('#new_group_title').bind('keyup', function () {
            $('#new_group_title_chars_left').text(30 - $(this).val().length);
        });
    }
